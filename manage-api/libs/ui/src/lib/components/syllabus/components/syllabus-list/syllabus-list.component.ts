...
import { SyllabusRepositoryService } from '@application/ui';
...
export class SyllabusListComponent implements OnInit, OnChanges {
    ...
    if (event== 'ALL') {
      ...
      if (event) {
        dataSource = this.list.filter((syllabus) => {
          return syllabus.type.toLowerCase().includes(event.toLowerCase());
        });
      } else {
  ...
  deleteChapters(){
    ...
    this.dataSource.filteredData.map(res=>{
      this.syllabusRepository.removeSyllabus(res).subscribe(data=>{})
    })
  }

  viewSyllabus(sub){
      this.syllabusRepository.getSubjectOrChapterList(sub.uuid).subscribe(res=>{
        const dialogRef = this.dialog.open(SyllabusInfoComponent, {
          data: { payload: res.response },
          height: '96%',
          width: '80%'
        });

...
function addLoadChildren(options) {
    return (host) => {
        ...
        workspace_1.insert(host, options.parentModule, [
            ...ast_utils_1.addRoute(options.parentModule, sourceFile, `{path: '${workspace_1.toFileName(options.fileName)}', loadChildren: () => import('${options.importPath}').then(module => module.${options.moduleName})}`),
        ]);

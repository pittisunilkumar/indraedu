...
export class VideosRepositoryService {
  ...
  getVideoByUuid(subjectUuid: string,chapterUuid:string, videoUuid: string): Observable<any> {
    return this.http.get(this.baseUrl + `videos/subjects/${subjectUuid}/chapter/${chapterUuid}/videos/${videoUuid}`);
  }

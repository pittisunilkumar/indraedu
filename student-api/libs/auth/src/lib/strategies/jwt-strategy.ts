...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(request, mobile: number, password: string): Promise<Employee> {
    ...
    return this.authService.validateUser(request).then((user) => {
      ...
      if (!user) {
        ...
        throw new HttpException({
          "status": false, "code": 2004, 'message': 'Unathorized', 'data': null
        }, HttpStatus.FORBIDDEN);
      }
  ...
  async validateEmployee(request, mobile: number, password: string): Promise<Employee> {
    const contextId = ContextIdFactory.getByRequest(request);
    // "AuthService" is a request-scoped provider
    // const authService = await this.moduleRef.resolve(AuthService, contextId);
    // console.log(mobile, password);
    ...
    return this.authService.validateEmployee(request).then((user) => {
      if (!user) {
        throw new UnauthorizedException();
      }

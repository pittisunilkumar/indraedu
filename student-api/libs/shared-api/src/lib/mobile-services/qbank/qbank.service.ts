...
export class MobileQbankService {
  ...
  async findSubjectsByCourse(courseId: string): Promise<any> {
    ...
    return this.courseModel
      .find({ uuid: courseId, 'flags.qBank': true }, { syllabus: 1, count: 1 })
      .populate({
        path: 'syllabus',
  ...
  async startTopic(startTopicDTO: StartTopicDTO, request): Promise<any> {
    ...
    return this.userModel.findOneAndUpdate(
      { uuid: request.user.uuid },
      {
        $addToSet: {
          "qbanksTestSubmissions":  {
            qbankTopicUuid: startTopicDTO.qbankTopicUuid,
      ...
      (res) => {
        return res.qbanksTestSubmissions;
      },
      (err) => {
        return err;
      }
  ...
  async stopTopic(stopTopicDTO: StopTopicDTO,request): Promise<any> {

    var userdata =  await this.userModel
    .aggregate([
      { $match: { uuid: request.user.uuid } },
      { $unwind: "$qbanksTestSubmissions" },
      {
        $match: {
          "qbanksTestSubmissions.qbankTopicUuid": stopTopicDTO.qbankTopicUuid
        }
      },
      {
        $project: {
    ...
    console.log(userdata);
      if(userdata){
        stopTopicDTO.startedAt = userdata;
      }
    return this.userModel.findOneAndUpdate(
      { uuid: request.user.uuid,  },
      // filter and replace the object
      {
        $set : { "qbanksTestSubmissions.$[elem]": stopTopicDTO },
      },
      { arrayFilters: [ { "elem.qbankTopicUuid": { $eq: stopTopicDTO.qbankTopicUuid } } ] },
    ).exec().then(
      (res) => {
        return res.qbanksTestSubmissions;
      },

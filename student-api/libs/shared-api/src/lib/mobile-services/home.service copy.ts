import { AboutInterface, HomePageDataInterface, QBankInterface, UserVideoInterface, VideoInterface } from '@application/api-interfaces';
import { Injectable } from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import { Banner, Course, User, QBank, QBankSubject, Test, Video, VideoSubject, SuggestedVideos, SuggestedTests, SuggestedQbank, SubmittedQBankTopic, TSCategories, SubmittedTest, MCQOfTheDay,Subscription, UserNotifications, DisableUserForTestSubmit } from '../schema';
...
export class MobileHomeService {
  ...
  constructor(
    ...
    @InjectModel('UserNotifications') private userNotifications: Model<UserNotifications>,
  ...
  async getData(request, body): Promise<HomePageDataInterface> {
    ...
    const banners = await this.bannersModel
      ...
      .sort('order')
      .exec();
    const mcqOfTheDay = await this.mcqOfTheModel.aggregate([
      { $match: { courseId: mongoose.Types.ObjectId(courseId), status: true } },
      {
        $lookup: {
          "from": "questions",
    ...
    const qbanks = [];
    const qbankList = await this.suggestedQbankModel.aggregate([
      { $match: { courseId: mongoose.Types.ObjectId(courseId), status: true } },
    ]).exec();
    for (var i = 0; i < qbankList.length; i++) {
      ...
      try {
        var userdataSubmittedTopic = await this.submittedQBankTopicModel
          .findOne({ "userId": userId, qbankTopicUuid: qbankUuid },
            {
              status: 1,
              stats: 1
            })
      ...
      var qbankData = await this.qbankSubjectModel
        ...
        qbankData.chapters.topics[0]['totalMcq'] = qbankData.chapters?.topics[0].que.length;
    ...
    const tests = [];
    const testList = await this.suggestedTestsModel.find(
      { courseId: courseId, status: true })
      .exec();
    for (var j = 0; j < testList.length; j++) {
      ...
      var userSubmittedTests = await this.submittedTestModel
        .findOne({
          "userId": userId, testSeriesUuid: testUuid, courseId: courseId
        },
          {
            status: 1,
            stats: 1
      ...
      if(submitStatus != 2){
        var testSeriesData = await this.testSeriesModel
          ...
          .exec()
          .then(res => {
            let totalres = res.filter(it => it.categories.tests.length > 0)[0];
            return totalres;
          })
          .catch(err => err);
          if (testSeriesData) {
    ...
    const videos = await this.suggestedVideosModel.aggregate([
      { $match: { courseId: mongoose.Types.ObjectId(courseId), status: true } },
    ]).exec();
    ...
    for (var k = 0; k < videos.length; k++) {
      ...
      if (videoStatus != 2) {
        var videoss = await this.videoSubjectModel
          ...
          .then(res => {
            var totalres
            let totalress = res.filter(it => it.chapters.videos.length > 0)[0].chapters.videos[0];
            // console.log(totalress)
            try{
              res.map(its => {
                if(its.chapters.videos.length > 0){
                  if(its.chapters.videos[0].slides != its.chapters.videos[0].suggestedBanner){
    ...
    return {
      "status": true, "code": 2000, 'message': 'Home Page Data', 'data': {
        banners,
  ...
  async findAllCourses(request): Promise<ApiResponseInterface> {
    try {

      var users = request.user
      var active = true;
      if(users){
        var disableUser = await this.disableUserForTestSubmitModel.findOne({"mobile":users.mobile, status:true, showInActiveCourses:true}).exec();
        if(disableUser){
          active = false
        }
      }
      ...
      if(active){
        ...
        var data = await this.courseModel
        .find({ 'flags.active': false }, { title: 1, uuid: 1, id: 1, imgUrl: 1 })
        .sort('order')
        .exec();
      }

      return { "status": true, "code": 2000, 'message': 'Courses Fetched', 'data': data }
    } catch (error) {
      return { "status": false, "code": 5000, 'message': 'Server error', 'data': {} }
    }
  }
  ...
  public getBannersByCourseId(courseId) {
    ...
    return this.bannersModel.aggregate([
  ...
  async UpdateUserSubscription(req, payload){
    var  subscriptionId = payload.subscriptionId ;
          ...
          UserData.subscriptions.map(sub => {
            if(sub.subscription_id.toHexString() == subscriptionData._id.toHexString()){
    ...
    return { "status": true, "code": 2000, 'message': 'User Subscription Data Updated', 'data': {

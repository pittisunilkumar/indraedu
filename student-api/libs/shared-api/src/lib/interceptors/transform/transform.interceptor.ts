import { ResponseInterface,NewResponseInterface } from '@application/api-interfaces';
import { CallHandler, ExecutionContext, Injectable, NestInterceptor } from '@nestjs/common';
import { Observable, throwError } from 'rxjs';
import { catchError, map } from 'rxjs/operators';

@Injectable()
export class TransformInterceptor<T> implements NestInterceptor<T, NewResponseInterface<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<NewResponseInterface<T>> {
    ...
    return next.handle().pipe(
      map(response => {
        ...
        if(context.switchToHttp().getRequest().originalUrl.includes('vdocipher')) {
          return ({ status: response.status, statusCode:response.code, message:response.message, data: response.data })
        } else {
